...
export const Verification = () => {
  ...
  useEffect(() => {
    const fetchVerifications = async () => {
      try {
        ...
        setQueueError('');
        const companyResult = await api.adminGetCompanyVerifications();
        setCompanyQueue(Array.isArray(companyResult) ? companyResult : []);
      } catch (error) {
        console.error(error);
  ...
  const historyItems = useMemo(() => {
    const merged = normalizedCompanyQueue
  ...
  }, [normalizedCompanyQueue, settings?.name]);

  const visibleQueue = useMemo(() => {
    if (!isCompanyUser) return normalizedCompanyQueue;

    let result = normalizedCompanyQueue;
    if (user?.email) {
      const email = user.email.toLowerCase();
      result = result.filter(item => (item.email || '').toLowerCase() === email);
    }
    return result;
  }, [normalizedCompanyQueue, isCompanyUser, user?.email]);

  const stats = useMemo(() => {
    const totalVerified = visibleQueue.filter(item => item.status === 'approved').length;
    const totalRejected = visibleQueue.filter(item => item.status === 'rejected').length;
    const totalPending = visibleQueue.filter(item => item.status === 'pending').length;

    const reviewedItems = visibleQueue.filter(item => item.reviewed_at_raw);
    ...
    return {
      ...
      totalRejected,
      totalPending,
      avgResponse: reviewedItems.length > 0 ? `${avgHours.toFixed(1)}h` : 'N/A'
    };
  }, [visibleQueue]);

  const ENTITIES = useMemo(() => {
    return Array.from(new Set(visibleQueue.map(item => item.name)));
  }, [visibleQueue]);
  ...
  const filteredAndSortedQueue = useMemo(() => {
    let result = visibleQueue.filter(item => {
      ...
      return matchesSearch && matchesIndustry && matchesStatus && matchesCompany;
    });
    ...
    <div className="flex flex-col h-full max-w-7xl mx-auto w-full p-8 gap-8 overflow-y-auto no-scrollbar">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-text-secondary text-lg">Manage and validate enterprise credentials.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex flex-col items-end">
            <span className="text-2xl font-black text-text-primary tracking-tight">
              {visibleQueue.filter(i => i.status === 'pending').length} Pending
            </span>
                ...
                <AnimatePresence>
                  {isCompanyFilterOpen && (
                    <>
                      ...
                      >
                        <div className="flex flex-col gap-3">
                          <p className="text-xs font-black text-text-secondary uppercase tracking-widest">Companies</p>
                    ...
                    <AnimatePresence mode="wait">
                      {showSuccess ? (
                        ...
                        >
                          ...
                          >
                            <CheckCircle2 className="size-5" /> Approve Entity
                          </button>
                          <button 
                            ...
                            <div className="flex items-center gap-5">
                              ...
                              <div className="flex flex-col gap-1.5">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border bg-blue-500/5 text-blue-600 border-blue-500/20">
